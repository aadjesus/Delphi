...
{
09/11/2006 - Paula  - Nova senha usuário Globus
05/04/2006 - Silvio - Nova senha para o segurança.
20/02/2006 - Silvio - Quando é carregado a partir do PORTAL, o menu principal não fica maximizado.
18/02/2006 - Fabiano - SIM 13814 - Implementar o controle de acesso ao sistema do Globus Report
                       conforme condições implementadas no cadastro de usuários no módulo de Controle.
14/02/2006 - Silvio - CTRNovidadesDoModulo.
20/09/2005 - Verônica - SIM 12360 - Alterado a forma de ler o CheckSum do menu especifico. Passa a
                        fazer o checkSum usando Sistema + Caption + Nome + "N". Antes no lugar do
                        Caption usava o Indice, mas quando o item de menu tinha a posição alterada,
                        perdia a autorização do menu especifico.
...
type
  ...
  private
    ...
    FPMenuGeral : TPopupMenu ;
    FMenuItem   : Array[0..6] of TMenuItem ;

    vUtilizaAtalhos,
    vUtilizaCheckList,
    vUtilizaLembretes,
    vUtilizaNovidadesMod,
    vUtilizaRemitirRel : Boolean ;
    ...
    procedure FPMenuGeralPopup(Sender: TObject);
    Procedure FIMenuClick(Sender: TObject) ;
    Procedure MostraLembretes ;
...
Uses
  MenuPrincipal, Bgm_Tela,
  CTR_CadastroDeCheckList,
  CTR_CadastroDeLembretes,
  CTR_CadastroDeAtalhos,
  CTR_RemitirRelatorio,
  CTR_NovidadesDoModulo ;
...
Var
  i : Integer ;
const
  vMenuItemCaption : Array[0..6] of String = ('Cadastro de atalhos','Check-List'     ,'Cadastro de lembretes', 'Novidades do módulo','Remitir relatório RD',    'Alterar lembrete',     'Excluir lembrete' ) ;
  vMenuItemName    : Array[0..6] of String = ('FIMenuTeclas'       ,'FIMenuCheckList','FIMenuCadastraLembrete','FIMenuNovidades',    'FIMenuRemitirRelatorioRD','FIMenuAlterarLembrete','FIMenuExcluirLembrete') ;
begin
  ...
  FPMenuGeral         := TPopupMenu.Create(Self) ;
  FPMenuGeral.Name    := 'FPMenuGeral' ;
  FPMenuGeral.OnPopup := FPMenuGeralPopup ;

  For i := 0 to Length(FMenuItem)-1 do
  Begin
    FMenuItem[i]         := TMenuItem.Create(Self) ;
    FMenuItem[i].Caption := vMenuItemCaption[i] ;
    FMenuItem[i].Name    := vMenuItemName[i] ;
    FMenuItem[i].OnClick := FIMenuClick ;
    FPMenuGeral.Items.Add(FMenuItem[i]) ;
  End ; // For i := 0 to Length(FMenuItem)-1 do

  vMDTeclas         := TRxMemoryData.Create(Nil) ;
  vMDTeclas.Name    := 'vMDTeclas' ;
  vMDTelas          := TRxMemoryData.Create(Nil) ;
  vMDTelas.Name     := 'vMDTelas' ;
  vMDCheckList      := TRxMemoryData.Create(Nil) ;
  vMDCheckList.Name := 'vMDCheckList' ;

  SetButtonsHeight(FButtonsHeight);
  SetButtonsWidth(FButtonsWidth);

  If (Not RodandoRemoto) And (dm <> nil) Then
  Begin
    dm.vPanelAux := TPanel.Create(Self) ;
    With dm.vPanelAux Do
    Begin
      Align      := alClient ;
      BevelOuter := bvNone ;
      Parent     := frmMenuPrincipal ;
    End ; // With dm.vPanelAux Do
    frmMenuPrincipal.Image1.Parent := dm.vPanelAux ;
    frmMenuPrincipal.Image2.Parent := dm.vPanelAux ;
  End ; // If Not RodandoRemoto Then
...
(*  vQuery := TQuery.Create(Self);
  ...
  { Para o usuário GLOBUS e para o módulo Globus Report, não mostra as novidades, pois este usuário não existe no cliente }
    ...
    Begin
      ...
      Begin
        ...
        Begin
           ...
           { Evita segunda entrada nesta rotina, pois na tela das novidades o form já estará aberto e
             posicionado na primeira mensagem a ser lida }
           vValidaoMenuNovamente := False;
           Msg;
           { Exibe a tela das novidades do módulo }
           ShowModal;
        End;
      End;
      Msg;
  ...
  If vUtilizaLembretes And TabelaExiste('CTR_Lembretes') Then
  Begin
    ApagaLembretes(Date,True) ;
    MostraLembretes ;
  End ; // If vUtilizaLembretes And ...
end;
...
//        edSenha.Text := '4.18767507002801'
    ...
    Begin
      { Quando é carregado a partir do PORTAL, o menu principal não fica maximizado - Silvio 20/02/2006 }
...
begin
  ...
  begin
    ...
    Open ;
    If (Trim(Sistema) = 'GRP') And ( edUsuario.Text <> 'GLOBUS' ) And ( edUsuario.Text <> 'MANAGER' ) And
       (FieldByName('ACESSOGLBREPORT').AsInteger = 1) then
    begin
      MensagemDlg('Usuário sem permissão a utilizar este módulo.', mtErro, mbOk );
      edUsuario.Clear;
      edUsuario.SetFocus;
    ...
    If FindField('UTILIZACADTECLAS'   ) <> Nil Then
      vUtilizaAtalhos := FieldByName('UTILIZACADTECLAS'        ).AsString = 'S' ;
    If FindField('UTILIZACADCHECKLIST') <> Nil Then
      vUtilizaCheckList := FieldByName('UTILIZACADCHECKLIST'   ).AsString = 'S' ;
    If FindField('UTILIZACADLEMBRETES') <> Nil Then
      vUtilizaLembretes := FieldByName('UTILIZACADLEMBRETES'   ).AsString = 'S' ;
    If FindField('UTILIZANOVIDADESMOD') <> Nil Then
      vUtilizaNovidadesMod := FieldByName('UTILIZANOVIDADESMOD').AsString = 'S' ;
    If FindField('UTILIZAREMITIRREL'  ) <> Nil Then
      vUtilizaRemitirRel := FieldByName('UTILIZAREMITIRREL'    ).AsString = 'S' ;
    If FindField('ENVIAREMAILDATELA'  ) <> Nil Then
      vcEnviarEMailDaTela:= FieldByName('ENVIAREMAILDATELA'    ).AsString = 'S' ;
...
//    vSenhaGLOBUS := '4.18767507002801';
      vSenhaGLOBUS := '4.10084033613445';
      vSenhaSEG    := '3.22128851540616' ;
      vSenhaSEG    := '5.10364145658263' ;
//    vSenhaSEG := '3.4453781512605';
  ...
  If vUtilizaAtalhos Or vUtilizaCheckList Or vUtilizaLembretes Or vUtilizaNovidadesMod Or vUtilizaRemitirRel Then
    dm.vPanelAux.PopupMenu := FPMenuGeral
...
//    sql.Add('SELECT * FROM CTR_MENUSDOSISTEMA');
    Sql.Add('SELECT A.*,0 AS ABSOLUTEINDEX,''N'' AS GRAVATECLAS,TECLAS AS COPIATECLAS FROM CTR_MENUSDOSISTEMA A');
    ...
    {$IFDEF EmDesenvolvimento}
      If FindField('TECLAS') <> Nil Then
      Begin
        vMDTeclas.LoadFromDataSet(Qry,0,lmCopy) ;
        vMDTelas.CopyStructure(vMDTeclas) ;
        vMDTelas.Open ;
      End ; // If FindField('TECLAS') <> Nil Then
    {$ENDIF}
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        If (Pos('ULTIMO',UpperCase(Menu.Items[M].Items[I].Items[S].Name)) > 0) AND
           (Pos('ACESSO',UpperCase(Menu.Items[M].Items[I].Items[S].Name)) > 0) Then
          Menu.Items[M].Items[I].Items[S].Visible := False;
      end;
      If (edUsuario.Text = 'GLOBUS') and (not vTemMenusEspecificosNaoAutorizados) then
        Menu.Items[M].Items[I].Visible := True
      else
        Menu.Items[M].Items[I].Visible := True And Menu.Items[M].Items[I].Enabled;
      If (Pos('ULTIMO',UpperCase(Menu.Items[M].Items[I].Name)) > 0) AND
         (Pos('ACESSO',UpperCase(Menu.Items[M].Items[I].Name)) > 0) Then
        Menu.Items[M].Items[I].Visible := False;
    end;
    If (edUsuario.Text = 'GLOBUS') and (not vTemMenusEspecificosNaoAutorizados) then
      Menu.Items[M].Visible := True
    else
      Menu.Items[M].Visible := True And Menu.Items[M].Enabled;
    If (Pos('ULTIMO',UpperCase(Menu.Items[M].Name)) > 0) AND
       (Pos('ACESSO',UpperCase(Menu.Items[M].Name)) > 0) then
      Menu.Items[M].Visible := False;
  end;
  { Habilita ToolBar }
  Qry.Free;
...
//      GravaLog(Sistema, EdUsuario.Text, Now, 'Informou senha.');
  ...
  begin
    ...
    begin
      ...
      try
        Sql.Clear ;
        Sql.Add(' SELECT A.*,0 AS ABSOLUTEINDEX,''N'' AS GRAVATECLAS,TECLAS AS COPIATECLAS FROM CTR_MENUSAOUSUARIO A WHERE USUARIO = :PUSUARIO AND SISTEMA = :PSISTEMA');
        ParamByName('PUSUARIO').asString := Trim(EdUsuario.Text);
      ...
      {$IFDEF EmDesenvolvimento}
        If FindField('TECLAS') <> Nil Then
        Begin
          vMDTeclas.LoadFromDataSet(MenusAoUsuario,0,lmCopy) ;
          vMDTelas.CopyStructure(vMDTeclas) ;
          vMDTelas.Open ;
        End ; // If FindField('TECLAS') <> Nil Then
      {$ENDIF}
...
begin
  ...
  For i := 0 to 14 do
  Begin
    If TShape(FindComponent('vShapeLembrete_' + StrZero(i,2) )) <> Nil Then
       TShape(FindComponent('vShapeLembrete_' + StrZero(i,2) )).Free ;

    If TPanel(FindComponent('vPnlLembrete_' + StrZero(i,2) )) <> Nil Then
       TPanel(FindComponent('vPnlLembrete_' + StrZero(i,2) )).Free ;
  End ; // For i := 0 to 14 do
...
begin
  ...
  Try
    ...
    begin
      ...
      ParamByName('pSistema').asString := Sistema;
      Open;
      If (Not vcUsuarioManagerComSenhaUm) And
         (StrToDateTime(FieldByName('DataHoraMenus').AsString) >= StrToDateTime(Edit1.Text)) Then
      Begin
        If StrToDateTime(FieldByName('DataHoraMenus').AsString) > StrToDateTime(Edit1.Text) Then
          MensagemDlg('Atenção: Esta versão pode não ser a mais nova. Use com cautela.'+ #13+'Na dúvida, fale com o administrador.', mtInformacao, mbOk );
        qrAux.Close;
        qrAux1.Close;
        qrAux.Free;
        qrAux1.Free;
  ...
  begin
    ...
    begin
      ...
      qrAux1.ParamByName('pNome'      ).asString := FieldByName('Nome'      ).asString;
      qrAux1.ExecSQL;
      { Limpa da tabela Ctr_Ultimoacessomenu }
      qrAux1.Close;
      qrAux1.Sql.Clear;
      qrAux1.Sql.Add('SELECT * FROM Ctr_Ultimoacessomenu ' );
      Try
        qrAux1.Open;
        qrAux1.Close;
        qrAux1.Sql.Clear;
        qrAux1.Sql.Add('Delete From Ctr_Ultimoacessomenu '            );
        qrAux1.Sql.Add('Where Sistema    = :pSistema    And ');
        qrAux1.Sql.Add('      IndiceMenu = :pIndiceMenu And ');
        qrAux1.Sql.Add('      Nome       = :pNome '          );
        qrAux1.ParamByName('pSistema'   ).asString := FieldByName('Sistema'   ).asString;
...
begin
  ...
  //////////////{ Monta o comando Sql para fazer um update na tabela Ctr_Ultimoacessomenu} Se existir
          Close;
          Sql.Clear;
          Sql.Add('SELECT * FROM Ctr_Ultimoacessomenu ' );
          Try
            Open;
            Close;
            Sql.Clear;
            Sql.Add('Update Ctr_Ultimoacessomenu'               );
            Sql.Add('Set Caption         = :pCaption, '         );
            Sql.Add('    IndiceMenu      = :pIndiceMenu  '      );
            Sql.Add('Where usuario      = :pusuario   And ');
            Sql.Add('      Sistema      = :pSistema        And ');
...
procedure TContSenhaBgm.FPMenuGeralPopup(Sender: TObject);
begin
  FMenuItem[0].Visible := vUtilizaAtalhos      And (FPMenuGeral.PopupComponent = dm.vPanelAux) ;
  FMenuItem[1].Visible := vUtilizaCheckList    And (FPMenuGeral.PopupComponent = dm.vPanelAux) ;
  FMenuItem[2].Visible := vUtilizaLembretes    And (FPMenuGeral.PopupComponent = dm.vPanelAux) And (vQtdeLembretes <= 15) ;
  FMenuItem[3].Visible := vUtilizaNovidadesMod And (FPMenuGeral.PopupComponent = dm.vPanelAux) ;
  FMenuItem[4].Visible := vUtilizaRemitirRel   And (FPMenuGeral.PopupComponent = dm.vPanelAux) ;
//  FMenuItem[5].Visible := Pos('Lembrete_',FPMenuGeral.PopupComponent.Name) > 0 ;
  FMenuItem[5].Visible := FPMenuGeral.PopupComponent <> dm.vPanelAux ;
  FMenuItem[6].Visible := FMenuItem[5].Visible ;
end;

procedure TContSenhaBgm.FIMenuClick(Sender: TObject);
Var
  vDataHoraLembrete : TDateTime ;
begin
  FPMenuGeralPopup(Sender) ; // Apenas p/ atualizar os itens do menu
  If Not TMenuItem(Sender).Visible Then Exit ;

  If TMenuItem(Sender).Name = 'FIMenuTeclas' Then
    With TFrmCTR_CadastroDeAtalhos.Create(Self) Do
    Begin
      ShowModal ;
      Free ;
    End // With TFrmCTR_CadastroDeAtalhos.Create(Self) Do
  Else // If TMenuItem(Sender).Name = 'FIMenuTeclas' Then
  If TMenuItem(Sender).Name = 'FIMenuCheckList' Then
    With TFrmCTR_CadastroDeCheckList.Create(Self) Do
    Begin
      ShowModal ;
      Free ;
    End // With TFrmCTR_CadastroDeCheckList.Create(Self) Do
  Else // If TMenuItem(Sender).Name = 'FIMenuCheckList' Then
  If Pos('Lembrete',TMenuItem(Sender).Name) > 0 Then
  begin
    vDataHoraLembrete := 0 ;
    If TMenuItem(Sender).Name <> 'FIMenuCadastraLembrete' Then
    Begin
      vcRetorno1        := TLabel(FindComponent('vLblLembrete_' + Right(FPMenuGeral.PopupComponent.Name,2))).Caption ;
      vDataHoraLembrete := StrToDateTime(Copy(vcRetorno1,1,10) +' '+ Copy(vcRetorno1,15,10)) ;
    End ; // If TMenuItem(Sender).Name <> 'FIMenuCadastraLembrete' Then

    If TMenuItem(Sender).Name = 'FIMenuExcluirLembrete' Then
    Begin
      ApagaLembretes(vDataHoraLembrete,False) ;
      vDataHoraLembrete := -1 ;
    End Else // If TMenuItem(Sender).Name = 'FIMenuExcluirLembrete' Then
      With TFrmCTR_CadastroDeLembretes.Create(Self) Do
      Begin
        vDataHora := vDataHoraLembrete ;
        ShowModal ;
        vDataHoraLembrete := vDataHora ;
        Free ;
      End ; // With TFrmCTR_CadastroDeLembretes.Create(Self) Do

    If vDataHoraLembrete = -1 Then
      MostraLembretes ;
  End Else // If Pos(TMenuItem(Sender).Name...
  If TMenuItem(Sender).Name = 'FIMenuNovidades' Then
    MensagemDlg('Em desenvolvimento', mtInformacao, mbOk )
  Else // If TMenuItem(Sender).Name = 'FIMenuNovidades' Then
  If TMenuItem(Sender).Name = 'FIMenuRemitirRelatorioRD' Then
    With TFrmCTR_RemitirRelatorio.Create(Self) Do
    Begin
      ShowModal ;
      Free ;
    End ; // With TFrmCTR_RemitirRelatorio.Create(Self) Do
end;

Procedure TContSenhaBgm.MostraLembretes ;
Var
  i : Integer ;
  vQryPesuisa : TQuery ;
  ...
  Begin
    ...
    vAtencao := (vQryPesuisa.FieldByName('DATAATENCAO').AsDateTime <> 0) And
                (vQryPesuisa.FieldByName('DATAATENCAO').AsDateTime <= Now) ;
    ...
    Begin
      ...
      Shape       := stRoundRect ;
      Brush.Color := StringToColor(vQryPesuisa.FieldByName(IIf(vAtencao,'CORATENCAO','CORLEMBRETE')).AsString) ;
      ...
      Begin
        ...
        Font.Color := StringToColor(vQryPesuisa.FieldByName(IIf(vAtencao,'CORFONTATENCAO','CORFONTLEMBRETE')).AsString) ;
        Font.Size  := 8 ;
        Font.Name  := 'MS Sans Serif' ;
        Visible		 := True ;
        Font.Style := [] ;
        PopupMenu  := FPMenuGeral ;
        Hint       := IIf(vAtencao,                                            '','Atenção : ' + vQryPesuisa.FieldByName('DATAATENCAO' ).AsString) ;
        Hint       := IIf(vQryPesuisa.FieldByName('DATAVALIDADE').AsDateTime=0,'',IIf(Trim(Hint)='','',Hint + #13) +
                                                                                  'Validade: ' + vQryPesuisa.FieldByName('DATAVALIDADE').AsString) ;
        ShowHint   := Trim(Hint) <> '' ;
        BringToFront ;

        vPnlLembrete2 := TPanel.Create(Self) ;
        ...
        Begin
          ...
          Begin
            ...
            Align       := alTop ;
            Caption     := FormatDateTime('dd/mm/yyyy " as " hh:mm:ss',vQryPesuisa.FieldByName('DATALEMBRETE').AsDateTime) ;
        		ParentColor := True ;
        		Visible		 	:= True ;
          ...
          Begin
            ...
            Align       := alClient ;
            Text        := vQryPesuisa.FieldByName('LEMBRETE').AsString ;
            BevelOuter  := bvNone ;
            BorderStyle := bsNone ;
        		ParentColor := True ;
            Visible		  := True ;
...
Begin
  vQryPesuisa := TQuery.Create(Self) ;
  With vQryPesuisa do
  Begin
...
Procedure TContSenhaBgm.ApagaLembretes(pData: TDateTime; pApagaVencidos : Boolean) ;
Begin
  ...
  Begin
    ...
    SQL.Add(' WHERE'+
            '     USUARIO = :P_USUARIO'+
            ' AND SISTEMA = :P_SISTEMA'+
            ' AND ' + IIf(pApagaVencidos,'DATAVALIDADE <'
                                        ,'DATALEMBRETE =') + ' :P_DATA') ;
    ParamByName('P_USUARIO').AsString   := vcUsuario ;
    ParamByName('P_SISTEMA').AsString   := vcSistema ;
    ParamByName('P_DATA'   ).AsDateTime := pData ;
    ExecSQL ;
    If vcDatabase.InTransaction Then vcDatabase.Commit ;
    Free ;
  End ; // With TQuery.Create(Self) do
End ;
