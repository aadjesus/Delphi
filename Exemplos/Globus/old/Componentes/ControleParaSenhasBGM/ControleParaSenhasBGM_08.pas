...
uses
  ...
  CTRNovidadesDoModulo, RxMemDS, Messages ;
...
procedure TContSenhaBgm.FEdtTeclaKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  If ((Shift = [ssShift])  Or
      (Shift = [ssCtrl] )) And
...
procedure TContSenhaBgm.FFrmTelaTeclasFormClose(Sender: TObject;
...
procedure TContSenhaBgm.FFrmTelaTeclasFormKeyPress(Sender: TObject;
  var Key: Char);
begin
  vAbandonaExit := False ;
  vEsc          := False ;
...
begin
  ...
  If vMDTeclas.Locate('GRAVATECLAS','S',[LocaseInsensitive]) Then
    vGravar := true  // MensagemDlg('Confirma a gravação dos atalhos cadastrados',mtConfirmacao,mbSim+mbNao) = mbSim
  Else
    Exit ;
...
procedure TContSenhaBgm.FTViewMenuTeclasChange(Sender: TObject;
  Node: TTreeNode);
begin
