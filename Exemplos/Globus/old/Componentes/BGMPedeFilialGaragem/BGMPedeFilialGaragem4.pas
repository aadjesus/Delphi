...
********************************************************************************
27/07/2006 Verônica   Colocado no create para quando for D7 a propriedade
                      AutoComplete fique false. Esta propriedade não existe no D4.
10/02/2006 - Lima   - Ajuste para preencher ao propriedade POPUPMENU somente se
                      a variável vPMenuFilial for diferente de NIL.
27/01/2006 - Silvio - Inclusão do DESTRUCTOR.
...
Var
  i : Integer ;
const
  vMenuItemCaption  : Array[0..3] of String    = ( 'Razão social','Razão Social/Nome Fantasia',          'Nome fantasia', 'Nome Fantasia/Cidade/UF') ;
...
{$IFnDEF VER120} // Versão <> DELPHI 4
  { Esta propriedade estava fazendo com que ao digitar o número 1 ela trouzesse a empresa 100 }
  AutoComplete := False;
{$ENDIF}
...
begin
  ...
  If (vcUsuario = 'GLOBUS') Or
     (vcUsuario = 'MANAGER') Then // Não consiste pelo grupo
    Sql := 'SELECT ' +
           'F.CODIGOFL, '                  +
           'A.RSOCIALEMPRESA, '            +
           'A.CHECKSUMEMPRESA, '           +
...
begin
  ...
  begin
    MensagemDlg('Filial ' + StrZero(CodigoFG,3) + ' não autorizada para o sistema ' + vcSistema + '.', mtErro, mbOk );
    CodigoFG := 0;
    qryFilialGaragem.Close;
    Result := False;
    SetFocus;
