...
type
  ...
  private
    ...
    FMenuItem   : Array[0..4] of TMenuItem ;

    vUtilizaAtalhos,
    vUtilizaCheckList,
    vUtilizaRecados  : Boolean ;
...
Uses
  MenuPrincipal, Bgm_Tela, CTR_CadastroDeCheckList, CTR_CadastroDeAtalhos, CTR_RemitirRelatorio, CTR_NovidadesDoModulo ;
...
Var
  i : Integer ;
const
  vMenuItemCaption : Array[0..4] of String = ('Cadastro de atalhos','Check-List'     ,'Cadastro de recados','Novidades do módulo','Remitir relatório RD') ;
  vMenuItemName    : Array[0..4] of String = ('FIMenuTeclas'       ,'FIMenuCheckList','FIMenuRecados'      ,'FIMenuNovidades',    'FIMenuRemitirRelatorioRD') ;
begin
  ...
  vUtilizaAtalhos   := True ;
  vUtilizaCheckList := True ;
  vUtilizaRecados   := True ;
end;
...
Var
  ...
  vNovidades : TStringList ;
  i          : integer ;
  vQuery     : TQuery ;

Const
  (*
  Tópico                         Assunto                                            Data       Empr Texto
  123456789012345678901234567890 12345678901234567890123456789012345678901234567890 1234567890 1234 12345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890
  Cadastro de itens1             Teste de assunto das novidades1                    13/02/2006      Agora é o texto com o detalhamento da novidade em si. Este poderá ocupar todo o restante da linha, sem limitação de tamanho 1.
  Cadastro de itens2             Teste de assunto das novidades2                    13/02/2006 0039 Agora é o texto com o detalhamento da novidade em si. Este poderá ocupar todo o restante da linha, sem limitação de tamanho 2. Específico do cliente 39. Não poderá aparecer para nenhuma outra empresa.
  Cadastro de itens3             Teste de assunto das novidades3                    13/02/2006      Agora é o texto com o detalhamento da novidade em si. Este poderá ocupar todo o restante da linha, sem limitação de tamanho 3.
  *)
  { Definem a estrutura do arquivo }
  cTamanhoTopico = 30;
  cInicialAssunto = 1 + cTamanhoTopico + 1; // 32
  cTamanhoAssunto = 50;
  cInicialData = cInicialAssunto + cTamanhoAssunto + 1; // 83
  cTamanhoData = 10;
  cInicialEmpresa = cInicialData + cTamanhoData + 1; // 94
  cTamanhoEmpresa = 4;
  cInicialTexto = cInicialEmpresa + cTamanhoEmpresa + 1; // 99
begin
  vUsuario := Value;
  vcUsuario := Value;
  ...
  If false Then // (vcUsuario <> 'GLOBUS') and (vcSistema <> 'GRP') and TabelaExiste('CTR_NovidadesDosModulos') Then
  Begin
    Msg('Verificando novidades ...');
    vNovidades := TStringList.Create ;
    rStream := TResourceStream.Create(hInstance, 'News', RT_RCDATA) ;
    vNovidades.LoadFromStream(rStream) ;
    rStream.Free ;
    If vNovidades.Count > 2 Then
    Begin
      ...
      Begin
        ...
        AbrirQuery(vQuery, 'NovidadesDosModulo') ;
        vMDNovidades := TRxMemoryData.Create(Nil) ;

        vMDNovidades.LoadFromDataSet(vQuery,0,lmCopy) ;
        Free ;
      End ; // With QryPesquisa do

      For i := 2 to vNovidades.Count - 1 do
      Begin
        vMDNovidades.Append ;
        vMDNovidades.FieldByName('LEU'         ).AsString  := 'N' ;
        vMDNovidades.FieldByName('PRIORIDADE'  ).AsInteger := 0 ;
        vMDNovidades.FieldByName('TOPICO'      ).AsString  := Trim(Copy(vNovidades[i], 1,               cTamanhoTopico)) ;
        vMDNovidades.FieldByName('ASSUNTO'     ).AsString  := Trim(Copy(vNovidades[i], cInicialAssunto, cTamanhoAssunto)) ;
        vMDNovidades.FieldByName('DATANOVIDADE').AsString  := Trim(Copy(vNovidades[i], cInicialData,    cTamanhoData)) ;
        vMDNovidades.Post ;
      End ; // For i := 2 to vNovidades.Count - 1 do

      If vMDNovidades.Locate('LEU','N',[loCaseInsensitive]) Then
        With TFrmCTR_NovidadesDoModulo.Create(Self) Do
        Begin
          RMDNovidades.LoadFromDataSet(vMDNovidades,0,lmCopy) ;
          ShowModal ;
          Free ;
        End ; // With FrmCTR_NovidadesDoModulo.Create(Self) Do

    End ; // If vNovidades.Count > 2 Then

    vNovidades.Free ;
    Msg ;
  End ; // If (vcUsuario <> 'GLOBUS') And (...
end;
...
begin
  ...
  {$IFDEF EmDesenvolvimento}
    TForm(Self.Parent).PopupMenu := FPMenuGeral ;
  {$ENDIF}
...
begin
  ...
  begin
    ...
    If FindField('UTILIZACADRECADOS'  ) <> Nil Then
      vUtilizaRecados   := FieldByName('UTILIZACADRECADOS'  ).AsString = 'S' ;
    ...
    begin
      ...
      Begin
        If StrToDateTime(FieldByName('DataHoraMenus').AsString) > StrToDateTime(Edit1.Text) Then
          MensagemDlg('Atenção: Esta versão pode não ser a mais nova. Use com cautela. Na dúvida, fale com o administrador.', mtErro, mbOk );
        qrAux.Close;
        qrAux1.Close;
        qrAux.Free;
        qrAux1.Free;
...
begin
  FMenuItem[0].Visible := vUtilizaAtalhos And (Not vMDTeclas.IsEmpty) ;
  FMenuItem[1].Visible := vUtilizaCheckList ;
  FMenuItem[2].Visible := vUtilizaRecados ;
  FMenuItem[3].Visible := True ;
end;
...
begin
  ...
  Begin
    ...
    Begin
      ...
      Free ;
    End ;
  End Else // If TMenuItem(Sender).Name = 'FIMenuTeclas' Then
  ...
  Begin
    ...
    Begin
      ...
      Free ;
    End ;
  End Else // If TMenuItem(Sender).Name = 'FIMenuCheckList' Then
  If TMenuItem(Sender).Name = 'FIMenuRecados' Then
  ...
  Begin
    ...
    Begin
      ...
      Free ;
    End ;
  End ; // If TMenuItem(Sender).Name = 'FIMenuTeclas' Then
end;
