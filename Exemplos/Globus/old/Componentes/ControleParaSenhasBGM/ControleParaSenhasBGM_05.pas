...
uses
  ...
  CTRNovidadesDoModulo, CTRNovidadesDoModulo2, RxMemDS, Messages, ImgList ;
...
begin
  ...
  { Para o GLOBUS, não mostra as novidades }
  If Assigned(frmNovidadesDoModulo) and (vcUsuario <> 'GLOBUS') and TabelaExiste('CTR_NovidadesDosModulos') Then
  Begin
    Msg('Verificando novidades ...');
    rStream := TResourceStream.Create(hInstance, 'News', RT_RCDATA);
    frmNovidadesDoModulo.RichEdit1.Lines.LoadFromStream(rStream);
    rStream.Free;
    If frmNovidadesDoModulo.RichEdit1.Lines.Count > 2 Then // Existe algo para exibir.
    Begin
      i := 2; // Linha inicial de texto. Desconsidera o cabeçalho (0 e 1).
      vMensagemInicial := 0;
      vQtdDeMensagens := 0;
      While i <= frmNovidadesDoModulo.RichEdit1.Lines.Count - 1 Do
      Begin
        vCodigoCliente := StrToIntDef(Copy(frmNovidadesDoModulo.RichEdit1.Lines[i], cInicialEmpresa, cTamanhoEmpresa), 0);
        ...
        Begin
          ...
          Continue;
        End;
        frmNovidadesDoModulo.edtTopico.Text := Trim(Copy(frmNovidadesDoModulo.RichEdit1.Lines[i], 1, cTamanhoTopico));
        frmNovidadesDoModulo.edtAssunto.Text := Trim(Copy(frmNovidadesDoModulo.RichEdit1.Lines[i], cInicialAssunto, cTamanhoAssunto));
        If frmNovidadesDoModulo.UsuarioAindaNaoLeu Then
        Begin
          Inc(vQtdDeMensagens);
      ...
      Begin
         frmNovidadesDoModulo.i := vMensagemInicial; // Na tela das novidades, continua na linha encontrada.
         frmNovidadesDoModulo.vQtdDeMensagens := vQtdDeMensagens;
         ...
         frmNovidadesDoModulo.ShowModal;
      End;

      FrmCTRNovidadesDoModulo2 := TFrmCTRNovidadesDoModulo2.Create(Application) ;
      With FrmCTRNovidadesDoModulo2 do
      Begin
//        While i <= frmNovidadesDoModulo.RichEdit1.Lines.Count - 1 Do
//aqui
        ShowModal ;
        Free ;
      End ; // With FrmCTRNovidadesDoModulo do
    End;
    Msg;
  End;
end;
...
begin
  ...
  begin
    MessageDlg('Usuário sem permissão a utilizar este módulo.', mtError, [mbOk], 0 );
    edUsuario.Clear;
    edUsuario.SetFocus;
    Exit;
  end;
...
//          If Not PesquisaString( edSenha.Text, + IIf(Sistema='SEG', 'SEG', '') + 'GLB' + DiasDaSemana[DayOfWeek(Now)] + Copy(DateToStr(Now),1,2) ) Then
  ...
  //         'GLB' + DiasDaSemana[DayOfWeek(Now)] + Copy(DateToStr(Now),1,2) )) Then // Silvio 12/6/2002
        Begin
          MessageDlg('Senha inválida para GLOBUS', mtError, [mbOk], 0 );
          edSenha.SetFocus;
          Exit;
        End
      else
    ...
    begin
      ...
      begin
        ...
        begin
          MessageDlg('Senha inválida para ADMINISTRADOR', mtError, [mbOk], 0 );
          edSenha.SetFocus;
        end;
        Close;
      end;
...
begin
  ...
  begin
    ...
    Begin
      Close;
      MessageDlg('Usuário inativo. Consulte o administrador.', mtError, [mbOk], 0 );
      edUsuario.SetFocus;
      Exit;
    End;
    If FieldByName('Senha').asString <> Encripta( edSenha.Text, 10, SenhaUsuarioPosicional ) Then
    ...
    begin
      ...
      Begin
        ...
        MessageDlg('Sua senha deve ser trocada. Se não souber como trocá-la, consulte o administrador.', mtWarning, [mbOk], 0 );
      End;
      If (FieldByName('DataSenha').AsString <> '') And
         (Abs(FieldByName('DataSenha').AsDateTime - Now) >= 60)Then
      Begin
        GravaLog(Sistema, edUsuario.Text, Now, 'Senha superior ou igual a 60 dias, acesso bloqueado.');
        MessageDlg('Acesso ao módulo bloqueado. Troque sua senha.', mtWarning, [mbOk], 0 );
        Usuario := edUsuario.Text;
        edSenha.Clear;
        EdSenha.SetFocus;
    ...
    begin
      ...
      Begin
        ...
        MessageDlg('Sua senha não é trocada há mais de 30 dias. Se não souber como trocá-la, consulte o administrador.', mtWarning, [mbOk], 0 );
      End;
    end;

    { Verifica se a senha é a padrão }
    If FieldByName('Senha').asString = Encripta( '123', 10, SenhaUsuarioPosicional ) then
      MessageDlg('Sua senha está como padrão. Precisa ser mudada. Se não souber como trocá-la, consulte o administrador.', mtWarning, [mbOk], 0 );
...
begin
  ...
  Begin
    vGravar := False ;
    if MessageDlg('Confirma a gravação dos atalhos cadastrados',mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      vGravar := True ;
  End Else
    Exit ;
...
begin
  ...
  Begin
    ...
    Begin
      MessageDlg('Teclas já cadastradas.', mtError, [mbOk], 0 );
      FEdtTecla.Clear ;
      FEdtTecla.SetFocus ;
      Exit ;
