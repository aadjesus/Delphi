...
uses
  ...
  SysUtils, Classes, Controls, ExtCtrls, Buttons, WinTypes, Forms,
  Graphics, StdCtrls, Dialogs, DbTables, Menus, Bgm_String, comctrls,
  ToolWin, DB, dmControle, Bgm_Dados, BGM_Utils, CTRTrocaSenhaDeUsuario,
  RxMemDS, CTRNovidadesDoModulo, Messages,
...
(*  vQuery := TQuery.Create(Self);
  ...
  If (vcUsuario <> 'GLOBUS') and (vcSistema <> 'GRP') and TabelaExiste('CTR_Novidades'{'CTR_NovidadesDosModulos'}) Then
  Begin
    ...
    Begin
      ...
      begin
        Close;
        sql.Clear;
        Sql.Add('Select CodNovidade, Topico, Data, Assunto, ''N'' Lida'          );
        Sql.Add('  FROM CTR_Novidades'                                           );
        Sql.Add(' Where CODNOVIDADE Not In (Select N.CODNOVIDADE'                );
        Sql.Add('                             From CTR_NOVIDADES N'              );
        Sql.Add('                                , CTR_NOVIDADESLIDAS L'         );
        Sql.Add('                            Where L.CODNOVIDADE = N.CODNOVIDADE');
        Sql.Add('                              And USUARIO = :PUsuario'          );
        Sql.Add('                              And Sistema = :pSistema)'         );
        Sql.Add('   And Sistema = :pSistema'                                     );
        Sql.Add('   And (concedente = :pConcedente Or concedente Is Null)'       );
        ParamByName('pUsuario'   ).AsString  := vcUsuario;
        ParamByName('pSistema'   ).AsString  := vcSistema;
        ParamByName('pConcedente').AsInteger := vcCodigoCliente;
        Open;
        RxM_Novidades.Emptytable;
        RxMLidas.EmptyTable;
        RxM_Novidades.LoadFromDataSet(Qrnovidade,0,lmCopy);
        RxMLidas.CopyStructure(RxM_Novidades);
        vNroMensagemAtual := 0;
        vQtdDeMensagens   := RecordCount;
        If not RxM_Novidades.IsEmpty then
        begin
          Inc(vNroMensagemAtual);
          RxM_Novidades.SortOnFields('Data');
          RxM_Novidades.First;
          PreencheEdit;
        end;
...
begin
  ...
  If ( ( Sistema = 'SEG' ) And ( edUsuario.Text <> 'GLOBUS' ) ) Or
       ...
       ( not VerificaSeUsuarioeAdministrador) and  // também habilita o controle para o usuário definido como administrador
       (not dm.vEnviaEMailLog)) then // se o usuario estiver logando para enviar e-mail de log de atualização, permite entrar..
  begin
...
//      GravaLog(Sistema, EdUsuario.Text, Now, 'Informou senha.');
  ...
  begin
    ...
    With Qry Do//With MenusAoUsuario Do
    begin
      Close;
      try
        Sql.Clear ;
  ...
  frmMenuPrincipal.vPrimeiraVez := True; //Para checar agenda interna 
end;
...
begin
  ...
  begin
    ...
    Begin
      vcRetorno1        := TLabel(dm.vPanelAux.FindComponent('vLblLembrete_' + Right(FPMenuGeral.PopupComponent.Name,2))).Caption ;
